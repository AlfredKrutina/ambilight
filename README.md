...
3. **Output**  
   ...
   - **Smart Home (optional):** **Home Assistant** `light.turn_on` with `rgb_color` (URL + long-lived token in **Settings → Smart Home**). The HA token is **not** stored inside `default.json`; it is written to application support as `ha_long_lived_token.txt` (same pattern as Spotify tokens). **Apple HomeKit** on **macOS** registers the same `MethodChannel` (`ambilight/homekit`), but the HomeKit Swift framework is not available to native AppKit targets — the channel returns **not supported**; use **Home Assistant** (or iOS) for HomeKit-driven lights from this stack. **Google Home** has no supported local desktop API—use Home Assistant’s [Google Assistant](https://www.home-assistant.io/integrations/google_assistant/) integration and control the same lights through HA.
