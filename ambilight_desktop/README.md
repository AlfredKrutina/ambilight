...
3. Spustí se workflow [`.github/workflows/desktop_release.yml`](../.github/workflows/desktop_release.yml): paralelně **Windows** (ZIP portable + **Inno Setup** `ambilight_desktop_windows.exe`) a **macOS** (nepodepsaný **DMG** `ambilight_desktop_macos.dmg`), pak job **`publish`** složí **`desktop-manifest.json`** (SHA-256 jen u ZIP pro auto-update) a všechny soubory nahraje na GitHub Release. ZIP musí obsahovat celý `Release/` včetně **`data/`**. Tagované `desktop-v*` nastaví GitHub **latest**; rolling `desktop-main` (push na větev) latest **neukradne** a do verze přidá `+{run_number}`.
4. Aplikace na stránce **Aktualizace** kontroluje manifest z výchozí URL Pages (`AMBI_DESKTOP_UPDATE_MANIFEST_URL` → `…/desktop/latest/desktop-manifest.json`, fallback GitHub `releases/latest`). Kanál v manifestu musí sedět s buildem (`AMBI_CHANNEL`, výchozí `stable`). Na Windows po ověření hashe a zápisu do složky instalace proběhne stažení, ukončení procesu a PowerShell skript přepíše soubory a znovu spustí `.exe` (portable / uživatelsky zapisovatelná složka — ne typicky Program Files bez elevation).

**GitHub „latest“:** Tagované desktop releasy mají `make_latest`; `desktop-main` ne. Pages mirror vybírá nejnovější `desktop-v*` / `desktop-main` podle data publikace.
