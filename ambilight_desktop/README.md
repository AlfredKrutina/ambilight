...
3. Spustí se workflow [`.github/workflows/desktop_release.yml`](../.github/workflows/desktop_release.yml): paralelně **Windows** (ZIP portable + **Inno Setup** `ambilight_desktop_windows_x64_setup.exe`) a **macOS** (nepodepsaný **DMG** `ambilight_desktop_macos.dmg`), pak job **`publish`** složí **`desktop-manifest.json`** (SHA-256 jen u ZIP pro auto-update) a všechny soubory nahraje na GitHub Release. ZIP musí obsahovat celý `Release/` včetně **`data/`**.
