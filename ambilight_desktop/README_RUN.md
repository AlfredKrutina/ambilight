...
- **HomeKit (Swift):** V **`Runner.xcodeproj` není** natvrdo **`HomeKit.framework`** v *Link Binary* — na CI by linker hlásil `framework 'HomeKit' not found`, i když Swift používá jen stub. Lokálně při plné kompilaci **`HomeKitChannel.swift`** Swift typicky **autolinkuje** HomeKit. Pokud u tebe *no such module*, přidej v Xcode framework ručně nebo `OTHER_LDFLAGS = -framework HomeKit` v konfiguraci. Na **GitHub Actions** **Podfile** při `pod install` nastaví **`AMBILIGHT_CI_NO_HOMEKIT`** → **`HomeKitChannelStub.swift`**, **`HomeKitChannel.swift`** se nezkompiluje.
...
- **DMG z CI:** `desktop_release` balí DMG přes **`tools/package_macos_dmg.sh`**: do obrazu jde staging se **`.app` + symlink „Applications“** (drag & drop), ne holý `-srcfolder` jen na `.app`. Pořád **nepodepsaný / ad-hoc** z CI; **notarizaci** řeš lokálně a nahraj stejný název souboru k release.
