...
/// Flash/OTA UI s výběrem konkrétního uloženého zařízení.
class DeviceFirmwarePanel extends StatefulWidget {
  const DeviceFirmwarePanel({
    super.key,
    required this.draft,
  ...
  @override
  State<DeviceFirmwarePanel> createState() => _DeviceFirmwarePanelState();
}

class _DeviceFirmwarePanelState extends State<DeviceFirmwarePanel> {
  ...
  List<DeviceSettings> get _devices => widget.draft.globalSettings.devices;

  DeviceSettings? get _selectedDevice {
  ...
  void initState() {
    ...
    _otaIpCtrl = TextEditingController();
    _otaPortCtrl = TextEditingController(text: '4210');

    final prefer = widget.initialDeviceId;
    if (prefer != null && _devices.any((d) => d.id == prefer)) {
      _selectedDeviceId = prefer;
    } else if (_devices.isNotEmpty) {
      _selectedDeviceId = _devices.first.id;
    }
    _applyDeviceToTargets(_selectedDevice, announce: false);
  ...
  @override
  void didUpdateWidget(covariant DeviceFirmwarePanel oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (widget.initialDeviceId != null &&
        widget.initialDeviceId != oldWidget.initialDeviceId &&
        _devices.any((d) => d.id == widget.initialDeviceId)) {
      setState(() {
        _selectedDeviceId = widget.initialDeviceId;
        _applyDeviceToTargets(_selectedDevice, announce: true);
      });
    }
    final stillExists = _selectedDeviceId != null && _devices.any((d) => d.id == _selectedDeviceId);
    if (!stillExists) {
      final nextId = _devices.isEmpty ? null : _devices.first.id;
      setState(() {
  ...
  void _applyDeviceToTargets(DeviceSettings? d, {required bool announce}) {
    if (d == null) {
      _otaIpCtrl.text = '';
      _otaPortCtrl.text = '4210';
      _selectedCom = null;
      return;
    }
    if (d.type == 'wifi') {
      _otaIpCtrl.text = d.ipAddress.trim();
      _otaPortCtrl.text = '${d.udpPort}';
    } else {
      final port = d.port.trim();
      if (port.isNotEmpty) {
        _selectedCom = port;
      }
      if (d.ipAddress.trim().isNotEmpty) {
        _otaIpCtrl.text = d.ipAddress.trim();
        _otaPortCtrl.text = '${d.udpPort}';
      }
    }
    if (announce && mounted) {
      ...
      _status = l10n.fwStatusFilledFromDevice(
        d.name.isEmpty ? d.id : d.name,
        d.type == 'wifi' ? d.ipAddress.trim() : d.port.trim(),
        d.type == 'wifi' ? '${d.udpPort}' : d.port.trim(),
      );
    }
  }
  ...
  void _refreshSerialPorts() {
    ...
    try {
      ...
      setState(() {
        ...
        final preferred = _selectedDevice?.port.trim();
        if (_selectedCom == null || !unique.contains(_selectedCom)) {
          if (preferred != null && preferred.isNotEmpty && unique.contains(preferred)) {
            _selectedCom = preferred;
          } else if (_selectedCom != null && !unique.contains(_selectedCom)) {
            _selectedCom = null;
          }
        }
      });
      ...
      assert(() {
        debugPrint('DeviceFirmwarePanel: SerialPort.availablePorts: $e\n$st');
        return true;
      }());
      if (!mounted) return;
      setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    final wifiFirst = selected?.type == 'wifi';
    ...
    if (devices.isEmpty) {
      return _section(
        child: Column(
          ...
          children: [
            Text(l10n.updatesNoDevicesTitle, style: Theme.of(context).textTheme.titleMedium),
            const SizedBox(height: 8),
            Text(
              l10n.updatesNoDevicesBody,
              style: Theme.of(context).textTheme.bodySmall?.copyWith(color: scheme.onSurfaceVariant),
            ),
            const SizedBox(height: 14),
            FilledButton.tonalIcon(
              onPressed: () => context.read<AmbilightAppController>().requestOpenDevices(),
              icon: const Icon(Icons.hub_outlined),
              label: Text(l10n.updatesGoToDevices),
            ),
          ],
        ),
      );
        ...
        _section(
          child: Column(
            ...
            children: [
              Row(
                children: [
                  Icon(Icons.devices_other_rounded, size: 22, color: scheme.primary),
                  const SizedBox(width: 10),
                  Expanded(
                    child: Text(l10n.updatesSelectDeviceLabel, style: Theme.of(context).textTheme.titleMedium),
                  ),
                ],
              ),
              const SizedBox(height: 8),
              Text(
                l10n.updatesSelectDeviceHint,
                style: Theme.of(context).textTheme.bodySmall?.copyWith(color: scheme.onSurfaceVariant),
              ),
              const SizedBox(height: 12),
              ValueListenableBuilder<Map<String, bool>>(
                valueListenable: context.read<AmbilightAppController>().connectionSnapshotNotifier,
                builder: (context, snap, _) {
                  return DropdownButtonFormField<String>(
                    value: _selectedDeviceId != null && devices.any((d) => d.id == _selectedDeviceId)
                        ? _selectedDeviceId
                        : null,
                    decoration: InputDecoration(
                      labelText: l10n.updatesSelectDeviceLabel,
                    ...
                    items: [
                      for (final d in devices)
                        DropdownMenuItem(
                          value: d.id,
                          child: Text(
                            _deviceMenuLabel(d, snap[d.id] == true, l10n),
            ...
            children: [
              Row(
                children: [
                  ...
                  const SizedBox(width: 10),
                  Expanded(
                    child: Text(l10n.fwManifestLabel, style: Theme.of(context).textTheme.titleMedium),
                  ),
                ],
              ),
              const SizedBox(height: 12),
        ...
        Theme(
          ...
          child: _section(
            child: ExpansionTile(
              ...
              title: Text(l10n.updatesAdvancedDebugTitle, style: Theme.of(context).textTheme.titleMedium),
              subtitle: Text(
  ...
  String _deviceMenuLabel(DeviceSettings d, bool online, AppLocalizations l10n) {
    final name = d.name.trim().isEmpty ? d.id : d.name.trim();
    final type = d.type == 'wifi' ? l10n.updatesDeviceTypeWifi : l10n.updatesDeviceTypeSerial;
    final state = online ? l10n.updatesDeviceOnline : l10n.updatesDeviceOffline;
    final detail = d.type == 'wifi'
        ? (d.ipAddress.trim().isEmpty ? '—' : d.ipAddress.trim())
        : (d.port.trim().isEmpty ? '—' : d.port.trim());
    return '$name · $type · $detail · $state';
  }

  Widget _usbFlashBody({
    required BuildContext context,
    required List<String> ports,
    required ColorScheme scheme,
    required AppLocalizations l10n,
    required FirmwareManifest? m,
  }) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
        Row(
          children: [
            ...
            const SizedBox(width: 10),
            Expanded(
              child: Text(l10n.fwFlashUsbTitle, style: Theme.of(context).textTheme.titleMedium),
            ),
            IconButton(
      ...
      children: [
        Row(
          children: [
            ...
            const SizedBox(width: 10),
            Expanded(
              child: Text(l10n.fwOtaUdpTitle, style: Theme.of(context).textTheme.titleMedium),
            ),
            if (emphasize)
              Chip(
                visualDensity: VisualDensity.compact,
                label: Text(l10n.updatesDeviceTypeWifi),
