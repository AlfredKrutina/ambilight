...
#define SERIAL_TIMEOUT_US     (10 * 1000000LL) // 10 seconds
...
static bool g_is_provisioned = false; // Flag to prevent auto-connect loops when unconfigured
...
esp_err_t load_wifi_creds(char* ssid, size_t ssid_len, char* pass, size_t pass_len, char* mqtt_uri, size_t uri_len, char* mqtt_user, size_t user_len, char* mqtt_pass, size_t mpass_len);
esp_err_t save_wifi_creds(const char* ssid, const char* pass, const char* mqtt_uri, const char* mqtt_user, const char* mqtt_pass);
...
void clear_tail_leds(int start_idx) {
    if (start_idx < LED_STRIP_NUM_LEDS) {
        memset(&led_colors[start_idx], 0, (LED_STRIP_NUM_LEDS - start_idx) * sizeof(rgb_t));
    }
}
...
"<style>"
":root{--bg:linear-gradient(135deg, #0f0c29, #302b63, #24243e);--primary:#0A84FF;--text:#fff;--text-sec:rgba(255,255,255,0.7);--glass:rgba(255, 255, 255, 0.05);--glass-border:rgba(255, 255, 255, 0.1);}"
...
".container{background:rgba(30, 30, 30, 0.6);backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px);padding:2.5rem;border-radius:24px;border:1px solid var(--glass-border);box-shadow:0 8px 32px rgba(0,0,0,0.5);width:100%;max-width:400px;animation:fadeIn 0.6s cubic-bezier(0.22, 1, 0.36, 1)}"
"@keyframes fadeIn{from{opacity:0;transform:translateY(20px) scale(0.95)}to{opacity:1;transform:translateY(0) scale(1)}}"
"h1{text-align:center;color:var(--primary);margin:0 0 0.5rem 0;font-weight:700;font-size:26px;text-shadow:0 0 20px rgba(10,132,255,0.3)}"
".subtitle{text-align:center;color:var(--text-sec);font-size:14px;margin-bottom:2rem}"
"label{display:block;margin-bottom:0.8rem;color:var(--text-sec);font-size:12px;font-weight:600;text-transform:uppercase;letter-spacing:1px}"
"input{width:100%;padding:16px;margin-bottom:1.5rem;background:rgba(0,0,0,0.2);border:1px solid var(--glass-border);border-radius:12px;color:#fff;font-size:16px;box-sizing:border-box;transition:0.3s}"
"input:focus{border-color:var(--primary);outline:none;background:rgba(0,0,0,0.4);box-shadow:0 0 0 4px rgba(10,132,255,0.1)}"
"button{width:100%;padding:16px;background:var(--primary);color:white;border:none;border-radius:12px;font-size:16px;font-weight:600;cursor:pointer;transition:0.3s;box-shadow:0 4px 15px rgba(10,132,255,0.3)}"
"button:hover{background:#0071e3;transform:translateY(-2px);box-shadow:0 6px 20px rgba(10,132,255,0.4)}"
"button:active{transform:translateY(1px)}"
"button.secondary{background:rgba(255,255,255,0.05);margin-top:1.5rem;color:var(--text);box-shadow:none;border:1px solid var(--glass-border)}"
"button.secondary:hover{background:rgba(255,255,255,0.1);transform:translateY(-2px)}"
".list-container{margin-top:0;max-height:0;overflow:hidden;transition:all 0.5s cubic-bezier(0.4, 0, 0.2, 1);border-radius:12px;background:rgba(0,0,0,0.2)}"
".list-container.open{max-height:300px;overflow-y:auto;margin-top:1.5rem;border:1px solid var(--glass-border)}"
".wifi-list{list-style:none;padding:0;margin:0}"
".wifi-item{display:flex;justify-content:space-between;align-items:center;padding:16px;border-bottom:1px solid var(--glass-border);cursor:pointer;transition:0.2s;animation:slideIn 0.3s ease-out forwards;opacity:0;transform:translateY(10px)}"
...
".signal-bars{display:flex;gap:2px;align-items:flex-end;height:12px;width:16px}"
...
".spinner{width:20px;height:20px;border:3px solid rgba(255,255,255,0.1);border-radius:50%;border-top-color:#fff;animation:spin 0.8s ease-in-out infinite;margin-right:10px;display:none}"
"@keyframes spin{to{transform:rotate(360deg)}}"
".footer{text-align:center;margin-top:2rem;font-size:11px;color:rgba(255,255,255,0.3);letter-spacing:1px;text-transform:uppercase}"
"</style></head><body>"
"<div class='container'>"
"<h1>Ambilight</h1>"
"<div class='subtitle'>Device Setup</div>"
"<form action='/save' method='post'>"
"<label>Wi-Fi Network</label><input id='ssid' name='ssid' placeholder='Select or Enter SSID' required autocomplete='off'>"
"<label>Password</label><input id='pass' name='pass' type='password' placeholder='Enter Password'>"
"<div class='subtitle' style='margin-top:20px;margin-bottom:10px'>Apple Home / MQTT (Optional)</div>"
"<label>MQTT Broker URI</label><input id='m_uri' name='m_uri' value='mqtt://192.168.1.126:1883' placeholder='mqtt://192.168.1.126:1883'>"
"<label>User</label><input id='m_user' name='m_user' value='mqtt-user' placeholder='User'>"
"<label>Password</label><input id='m_pass' name='m_pass' type='password' value='mqtt' placeholder='Password'>"

"<button type='submit'>Save & Connect</button>"
"</form>"
"<button class='secondary' id='scanBtn' onclick='scanWifi()'>"
"<div class='spinner' id='spinner'></div><span id='scanText'>Scan Networks</span>"
"</button>"
...
"let n=0;if(rssi>=-50)n=4;else if(rssi>=-60)n=3;else if(rssi>=-70)n=2;else if(rssi>=-80)n=1;"
"return `<div class='signal-bars'>`+[1,2,3,4].map(i=>`<div class='bar ${i<=n?'active':''}'></div>`).map((h,i)=>`<div style='height:${(i+1)*3}px' class='bar ${i<n?'active':''}'></div>`).join('')+`</div>`;"
"}"
"function scanWifi(){"
"const b=document.getElementById('scanBtn'),s=document.getElementById('spinner'),t=document.getElementById('scanText'),l=document.getElementById('wifiList'),c=document.getElementById('listContainer');"
"b.disabled=true;s.style.display='block';t.textContent='Scanning...';l.innerHTML='';c.classList.remove('open');"
"function poll(){"
...
"l.innerHTML=d.networks.map((n,i)=>`<li class='wifi-item' style='animation-delay:${i*0.05}s' onclick=\"select('${n.ssid.replace(/'/g, \"\\\\'\")}')\"><span class='wifi-name'>${n.ssid}</span><div class='wifi-info'>${n.auth>0?'<span class=\"lock\">🔒</span>':''}${getSignalBars(n.rssi)}</div></li>`).join('');"
"c.classList.add('open'); "
"b.disabled=false;s.style.display='none';t.textContent='Refresh Networks';"
"} }).catch(e=>{console.log(e);t.textContent='Scan Failed';b.disabled=false;s.style.display='none';});"
"}"
"poll();"
"}"
"function select(n){const i=document.getElementById('ssid');i.value=n;i.classList.add('pulse');setTimeout(()=>i.classList.remove('pulse'),300);document.getElementById('pass').focus();}"
...
":root{--bg:linear-gradient(135deg, #0f0c29, #302b63, #24243e);--primary:#0A84FF;--text:#fff;--text-sec:rgba(255,255,255,0.7);--glass:rgba(255, 255, 255, 0.05);--glass-border:rgba(255, 255, 255, 0.1);}"
"body{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;background:var(--bg);color:var(--text);display:flex;justify-content:center;align-items:center;min-height:100vh;margin:0;padding:20px;box-sizing:border-box}"
".container{background:rgba(30, 30, 30, 0.6);backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px);padding:2.5rem;border-radius:24px;border:1px solid var(--glass-border);box-shadow:0 8px 32px rgba(0,0,0,0.5);width:100%;max-width:400px;animation:fadeIn 0.6s cubic-bezier(0.22, 1, 0.36, 1);text-align:center}"
"@keyframes fadeIn{from{opacity:0;transform:translateY(20px) scale(0.95)}to{opacity:1;transform:translateY(0) scale(1)}}"
"h1{color:var(--primary);margin:0 0 0.5rem 0;font-weight:700;font-size:26px;text-shadow:0 0 20px rgba(10,132,255,0.3)}"
".subtitle{color:var(--text-sec);font-size:14px;margin-bottom:2rem}"
".spinner{width:50px;height:50px;border:4px solid rgba(255,255,255,0.1);border-radius:50%;border-top-color:var(--primary);animation:spin 1s ease-in-out infinite;margin:0 auto 1.5rem auto}"
"@keyframes spin{to{transform:rotate(360deg)}}"
".progress-container{width:100%;height:4px;background:rgba(255,255,255,0.1);border-radius:2px;overflow:hidden;margin-top:1.5rem}"
".progress-bar{height:100%;background:var(--primary);width:0%;transition:width 0.1s linear;box-shadow:0 0 10px rgba(10,132,255,0.5)}"
"</style></head><body>"
"<div class='container'>"
"<div class='spinner'></div>"
"<h1>Settings Saved</h1>"
"<div class='subtitle'>Device is restarting...<br>Please wait while changes apply.</div>"
"<div class='progress-container'><div class='progress-bar' id='bar'></div></div>"
"</div>"
"<script>"
"let p=0;const b=document.getElementById('bar');"
"const i=setInterval(()=>{p+=1;b.style.width=p+'%';if(p>=100){clearInterval(i);window.location.href='/';}},100);"
"</script></body></html>";
...
esp_err_t scan_get_handler(httpd_req_t *req) {
    ...
    if (g_scan_status == 0) {
        ...
        if(ap_count > 15) ap_count = 15; // Reduce logic to 15 to save heap

        wifi_ap_record_t *ap_list = (wifi_ap_record_t *)malloc(sizeof(wifi_ap_record_t) * ap_count);
        if (!ap_list) {
            ESP_LOGE(TAG, "OOM in scan_get_handler! Free Heap: %lu", esp_get_free_heap_size());
        ...
        httpd_resp_send_chunk(req, "{\"status\":\"complete\",\"networks\":[", HTTPD_RESP_USE_STRLEN);
        ...
        for (int i = 0; i < ap_count; i++) {
            ...
            if (strlen(ssid_safe) == 0) continue;
            snprintf(buf, sizeof(buf), "{\"ssid\":\"%s\",\"rssi\":%d,\"auth\":%d}", ssid_safe, ap_list[i].rssi, ap_list[i].authmode);
...
// ============ NVS HELPERS ============
esp_err_t save_wifi_creds(const char* ssid, const char* pass, const char* muri, const char* muser, const char* mpass) {
    ...
    if(muri && strlen(muri)>0) nvs_set_str(nvs_handle, "m_uri", muri);
    else nvs_erase_key(nvs_handle, "m_uri");
    
    if(muser && strlen(muser)>0) nvs_set_str(nvs_handle, "m_user", muser);
    else nvs_erase_key(nvs_handle, "m_user");
    
    if(mpass && strlen(mpass)>0) nvs_set_str(nvs_handle, "m_pass", mpass);
    else nvs_erase_key(nvs_handle, "m_pass");
...
esp_err_t load_wifi_creds(char* ssid, size_t ssid_len, char* pass, size_t pass_len,
                          char* muri, size_t muri_len, char* muser, size_t muser_len, char* mpass, size_t mpass_len) {
    nvs_handle_t nvs_handle;
    if (nvs_open("storage", NVS_READONLY, &nvs_handle) != ESP_OK) return ESP_FAIL;

    size_t required_size;
    
    // Load SSID
    if(nvs_get_str(nvs_handle, "ssid", NULL, &required_size) == ESP_OK && required_size <= ssid_len)
        nvs_get_str(nvs_handle, "ssid", ssid, &required_size);
    else { nvs_close(nvs_handle); return ESP_FAIL; }

    // Load Password
    if(nvs_get_str(nvs_handle, "pass", NULL, &required_size) == ESP_OK && required_size <= pass_len)
        nvs_get_str(nvs_handle, "pass", pass, &required_size);
    else { nvs_close(nvs_handle); return ESP_FAIL; }
    
    // Load MQTT (Optional) using safe defaults if missing
    // MIGRATION LOGIC: Check new keys ("m_*"), then fallback to old keys ("mqtt_*")
    
    if(muri) {
        if(nvs_get_str(nvs_handle, "m_uri", NULL, &required_size) == ESP_OK && required_size <= muri_len) {
            nvs_get_str(nvs_handle, "m_uri", muri, &required_size);
        } else if(nvs_get_str(nvs_handle, "mqtt_uri", NULL, &required_size) == ESP_OK && required_size <= muri_len) {
    ...
    if(muser) {
        if(nvs_get_str(nvs_handle, "m_user", NULL, &required_size) == ESP_OK && required_size <= muser_len) {
            nvs_get_str(nvs_handle, "m_user", muser, &required_size);
        } else if(nvs_get_str(nvs_handle, "mqtt_user", NULL, &required_size) == ESP_OK && required_size <= muser_len) {
    ...
    if(mpass) {
         if(nvs_get_str(nvs_handle, "m_pass", NULL, &required_size) == ESP_OK && required_size <= mpass_len) {
            nvs_get_str(nvs_handle, "m_pass", mpass, &required_size);
         } else if(nvs_get_str(nvs_handle, "mqtt_pass", NULL, &required_size) == ESP_OK && required_size <= mpass_len) {
...
esp_err_t erase_wifi_creds() {
    nvs_handle_t nvs_handle;
    if (nvs_open("storage", NVS_READWRITE, &nvs_handle) != ESP_OK) return ESP_FAIL;
...
esp_err_t save_post_handler(httpd_req_t *req) {
    ...
    char ssid_enc[33]={0}, pass_enc[65]={0}, ssid_dec[33]={0}, pass_dec[65]={0};
...
esp_err_t redirect_handler(httpd_req_t *req) {
    ...
    sprintf(redirect_url, "http://%d.%d.%d.%d/", octets[0], octets[1], octets[2], octets[3]);
...
void start_webserver(void) {
    ...
    if (httpd_start(&server, &config) == ESP_OK) {
        ...
        httpd_uri_t sc = { .uri = "/scan", .method = HTTP_GET, .handler = scan_get_handler };
        ...
        const char * captive_uris[] = {
            ...
            "/canonical.html", "/connecttest.txt", "/success.txt"
        };
        
        for (int i = 0; i < sizeof(captive_uris)/sizeof(captive_uris[0]); i++) {
             httpd_uri_t func_uri = { .uri = captive_uris[i], .method = HTTP_GET, .handler = redirect_handler };
             httpd_register_uri_handler(server, &func_uri);
        }

        // CATCH ALL for Captive Portal (redirects all other URLs to /)
        httpd_uri_t catch_all = { .uri = "/*", .method = HTTP_GET, .handler = redirect_handler };
...
static void event_handler(void* arg, esp_event_base_t base, int32_t id, void* data) {
    if (base == WIFI_EVENT && id == WIFI_EVENT_STA_START) {
        ...
        if (g_connection_retries > MAX_RETRIES) {
            ...
            // Visual Feedback: Red Flash (Failure)
            xSemaphoreTake(led_mutex, portMAX_DELAY);
            for(int k=0; k<3; k++) {
                for(int i=0; i<LED_STRIP_NUM_LEDS; i++) led_strip_set_pixel(led_strip, i, 255, 0, 0); // RED
        ...
        // Critical Fix: If we lost connection, ensure AP is back up so user can reach us
        // We might be in STA_ONLY mode, so switch back to APSTA
        ...
        if (g_is_provisioned) {
            ESP_LOGI(TAG, "Retry Connection (%d/%d)...", g_connection_retries, MAX_RETRIES);
        ...
        g_last_data_interaction = esp_timer_get_time(); // Init timer to prevent instant timeout
        
        // Init mDNS for .local resolution
        if (mdns_init() == ESP_OK) {
             mdns_hostname_set("ambilight-esp32");
             mdns_instance_name_set("Ambilight LED Controller");
             ESP_LOGI(TAG, "mDNS Init: ambilight-esp32.local");
        }

        // Start MQTT
        start_mqtt();

        // VISUAL INDICATION: Green Flash (3x)
        xSemaphoreTake(led_mutex, portMAX_DELAY);
        for(int k=0; k<3; k++) {
            for(int i=0; i<LED_STRIP_NUM_LEDS; i++) led_strip_set_pixel(led_strip, i, 0, 255, 0);
            led_strip_refresh(led_strip);
            vTaskDelay(pdMS_TO_TICKS(200));
            led_strip_clear(led_strip);
            led_strip_refresh(led_strip);
...
void start_wifi_subsystem() {
    ...
    if (boot_count > 5) {
        ESP_LOGE(TAG, "BOOT LOOP DETECTED (Count: %d). Forcing Safe Mode (AP Only).", boot_count);
        force_safe_mode = true;
        // Visual Indication: RED Slow Pulse
        xSemaphoreTake(led_mutex, portMAX_DELAY);
        for(int k=0; k<5; k++) {
            for(int i=0; i<LED_STRIP_NUM_LEDS; i++) led_strip_set_pixel(led_strip, i, 100, 0, 0); // Dim Red
            led_strip_refresh(led_strip);
    ...
    if (ret == ESP_OK) {
        ...
        if (ap_list && esp_wifi_scan_get_ap_records(&ap_count, ap_list) == ESP_OK) {
            ...
            while(conflict && suffix < 65534) { // Limit to 65534 tries
                ...
                if (suffix == 0) strcpy(unique_ssid, AP_SSID);
                else sprintf(unique_ssid, "%s_%d", AP_SSID, suffix);
            ...
            if (!conflict) ESP_LOGI(TAG, "Selected Unique SSID: %s", unique_ssid);
            else ESP_LOGW(TAG, "Could not find unique SSID, using default: %s", unique_ssid);
        }
    ...
    if (!force_safe_mode) {
         has_creds = (load_wifi_creds(ssid, sizeof(ssid), pass, sizeof(pass), 
                                      g_mqtt_uri, sizeof(g_mqtt_uri), 
                                      g_mqtt_user, sizeof(g_mqtt_user), 
    ...
    wifi_config_t ap_cfg = { .ap = { .ssid="", .ssid_len=0, .channel=1, .max_connection=4, .authmode=WIFI_AUTH_OPEN } };
    ...
    if (has_creds) {
        ...
        xSemaphoreTake(led_mutex, portMAX_DELAY);
        for(int i=0; i<LED_STRIP_NUM_LEDS; i++) led_strip_set_pixel(led_strip, i, 255, 140, 0); // Orange
        led_strip_refresh(led_strip);
        xSemaphoreGive(led_mutex);
    ...
    // DISABLE POWER SAVE to ensure low latency for UDP/Ambilight and reliable ping
...
void task_monitor(void* arg) {
    while(1) {
        ...
        if (serial_active && g_wifi_enabled) {
            // Priority: Serial. Kill Wi-Fi.
            stop_wifi_subsystem();
        } else if (!serial_active && !g_wifi_enabled) {
            // Timeout: Start Wi-Fi.
            start_wifi_subsystem();
        }

        // Timeout Logic for STA Mode (Idle Disconnect)
        if (g_wifi_enabled && !g_ap_disabled_after_connection) { // Wait, !g_ap_disabled means AP IS ENABLED. 
             // Logic Check: When connected, g_ap_disabled_after_connection = true.
             // So we want to check when it IS true (STA Only mode)
        }
        
        // Correct Logic: If we are in STA mode (AP disabled), check for data timeout
        if (g_wifi_enabled && g_ap_disabled_after_connection) {
             EventBits_t bits = xEventGroupGetBits(s_wifi_event_group);
             if (bits & WIFI_CONNECTED_BIT) {
                 // Check if Data Timeout exceeded AND we haven't actively used the device
                 if (!g_has_received_data && (esp_timer_get_time() - g_last_data_interaction) > DATA_TIMEOUT_US) {
                     // Check if MQTT Home Mode is active. If so, DO NOT disconnect.
                     if (g_mqtt_connected) {
                         // We are in Home Mode.
                     } else {
                         ESP_LOGW(TAG, "Idle Timeout (5min) & No Data & No MQTT. Disconnecting to AP Mode.");
                         g_suppress_reconnect = true; // Don't auto-reconnect
                         esp_wifi_disconnect();
                     }
                 }
                 
                 // Restore Home Color if UDP stops
                 if (g_mqtt_connected && (esp_timer_get_time() - g_last_data_interaction) > 2000000) { // 2s after last UDP
                        // We rely on the fact that UDP task is not updating LEDs.
                        // But we need to apply Home Color once.
                        // Implemented in UDP task loop or here? 
                        // Better: In UDP task, if timeout -> Do nothing (let MQTT handler update?). 
                        // Actually MQTT handler only updates ON EVENT.
                        // So we need to RE-APPLY Home Color once after UDP timeout.
                 }
             }
        }
...
void task_serial(void *arg) {
    usb_serial_jtag_driver_config_t c = { .rx_buffer_size = 2048, .tx_buffer_size = 1024 };
    ...
    while(1) {
        // Short timeout allows us to detect "End of Frame" silence
        ...
        if (len > 0) {
            ...
            for(int i=0; i<len; i++) {
                ...
                if (state == STATE_IDLE) {
                    if (b == 0xFF) {
                        ...
                        // CLEAR All LEDs at start of frame (handles surplus leds)
                        xSemaphoreTake(led_mutex, portMAX_DELAY);
                    ...
                    if (tuple_pos == 4) {
                        // Got full tuple: Idx, R, G, B
                        // Protocol is: [Idx, R, G, B]
                        // Note: If PC sends 0xFF prefix, and then data 0xFF... 
                        // We are in DATA state, so we consume it as data.
                        
                        uint8_t idx = tuple_buf[0];
                        uint8_t r   = tuple_buf[1];
                        uint8_t g   = tuple_buf[2];
                        uint8_t b_val = tuple_buf[3];
            ...
            if (state == STATE_DATA && dirty) {
                 ...
                 xSemaphoreGive(led_mutex);
                 dirty = false;
                 // Stay in DATA state? No, return to IDLE to sync next 0xFF
                 state = STATE_IDLE; 
            }
        }
    }
...
void task_udp(void *arg) {
    ...
    while (1) {
        ...
        while (g_wifi_enabled) { // Inner loop checks flag
            ...
            if (len <= 0) {
                 if(was_pc_mode && (esp_timer_get_time() - g_last_data_interaction) > 5000000) { // 5s Silence
                      ...
                      if(g_mqtt_connected) {
                          ...
                          esp_mqtt_client_publish(mqtt_client, state_topic, g_home_power ? "true" : "false", 0, 1, 0);
                      ...
                      if(g_mqtt_connected && g_home_power) {
                           xSemaphoreTake(led_mutex, portMAX_DELAY);
                           for(int i=0; i<LED_STRIP_NUM_LEDS; i++) led_strip_set_pixel(led_strip, i, g_home_color.r, g_home_color.g, g_home_color.b);
                           led_strip_refresh(led_strip);
                           xSemaphoreGive(led_mutex);
                      }
                 }
            ...
            if (len > 0) {
                if (!was_pc_mode) { 
                    // FEEDBACK: Notify HomeKit we are active (Ambilight is effectively Power ON)
                    if(g_mqtt_connected) {
                        char state_topic[64];
                        snprintf(state_topic, sizeof(state_topic), "alfred/devices/%s/power/state", g_device_id);
                ...
                if (strncmp(rx_buffer, "DISCOVER_ESP32", 14) == 0) {
                   ...
                   char resp[128];
                   ESP_LOGI(TAG, "Discovery Broadcast Received from %s", inet_ntoa(source_addr.sin_addr.s_addr));
                   sprintf(resp, "ESP32_PONG|%02x%02x%02x|Ambilight_%02x|%d|2.0", mac[3],mac[4],mac[5],mac[5], LED_STRIP_NUM_LEDS);
                   int sent = sendto(sock, resp, strlen(resp), 0, (struct sockaddr *)&source_addr, sizeof(source_addr));
                   if (sent < 0) ESP_LOGE(TAG, "Failed to send PONG: %d", errno);
                   else ESP_LOGI(TAG, "Sent PONG: %s", resp);
                   continue;
                }
                ...
                if (strncmp(rx_buffer, "IDENTIFY", 8) == 0) {
                    ...
                    // Flash Blue for 1s
                    xSemaphoreTake(led_mutex, portMAX_DELAY);
                    for(int i=0; i<LED_STRIP_NUM_LEDS; i++) led_strip_set_pixel(led_strip, i, 0, 0, 255); // Blue
                    led_strip_refresh(led_strip);
                    xSemaphoreGive(led_mutex);
                ...
                if (strncmp(rx_buffer, "RESET_WIFI", 10) == 0) {
                    ...
                    xSemaphoreTake(led_mutex, portMAX_DELAY);
                    for(int k=0; k<5; k++) {
                        for(int i=0; i<LED_STRIP_NUM_LEDS; i++) led_strip_set_pixel(led_strip, i, 255, 0, 0);
                        led_strip_refresh(led_strip);
                        vTaskDelay(pdMS_TO_TICKS(100));
                        led_strip_clear(led_strip);
                        led_strip_refresh(led_strip);
                ...
                if (len >= 6 && (uint8_t)rx_buffer[0] == 0x03) { // Single Pixel [0x03, IdxH, IdxL, R, G, B]
                    ...
                    if (idx < LED_STRIP_NUM_LEDS) {
                         ...
                         led_colors[idx].g = g;
                         led_colors[idx].b = b_val;
                         // Update strip immediately with max brightness for calibration visibility
                         update_leds(255); 
                         xSemaphoreGive(led_mutex);
                    }
                    continue;
                ...
                if (len >= 4 && (uint8_t)rx_buffer[0] == 0x02) { // Binary Frame [0x02, Bri, R, G, B...]
                    ...
                    if (g_controller_ip != 0 && g_controller_ip != sender_ip) {
                        ...
                        if (drop_log++ % 100 == 0) {
                            ESP_LOGW(TAG, "Dropped packet from unauthorized source (Locked).");
                        }
                        continue; 
                    }
                    ...
                    // Packet Format: [0x02, Bri, Seq, R, G, B...] (New Format) vs [0x02, Bri, R, G, B...] (Old)
                    // We can heuristically detect if byte 2 is a sequence number if we assume old format rarely changes.
                    // BETTER: Just assume unsequenced for now, but enforces strict timing.
                    
                    // Simple Jitter Fix: If packet is older than last one (reordered), drop it.
                    // Since we don't have seq num in current protocol, we rely on arrival time.
                    // But UDP arrival time IS the order we see it.
                    
                    // The issue is likely partial updates or buffer tearing?
                    // UDP ensures full message or nothing.
                    
                    // Real Issue: Direct immediate update causes tearing if update rate > strip refresh rate.
                    // Fix: Enforce Min Frame Time (e.g. 15ms ~ 60fps).
                    ...
                    // If we receive valid data, the system is working. Reset crash counter.
                    ...
                    int num = (len - 2) / 3;
                    if(num > LED_STRIP_NUM_LEDS) num = LED_STRIP_NUM_LEDS;
...
void task_dns_server(void *arg) {
    ...
    while(1) {
         if (!g_wifi_enabled) { vTaskDelay(pdMS_TO_TICKS(1000)); continue; }
         ...
         while(g_wifi_enabled) {
             ...
             if (len > 0) {
                 ...
                 // Flags (Bytes 2-3): Standard Response (0x8000) | Authoritative (0x0400) | No Error
                 // 0x8180 = QR(1)|Op(0000)|AA(1)|TC(0)|RD(1) | RA(1)|Z(0)|RCODE(0)
                 rx_buffer[2] = 0x81;
                 rx_buffer[3] = 0x80;
                 ...
                 if (ptr <= sizeof(rx_buffer) - 16) {
                     ...
                     rx_buffer[ptr++] = 0xC0; rx_buffer[ptr++] = 0x0C;
                     // TYPE: A (0x0001)
                     rx_buffer[ptr++] = 0x00; rx_buffer[ptr++] = 0x01;
                     // CLASS: IN (0x0001)
                     rx_buffer[ptr++] = 0x00; rx_buffer[ptr++] = 0x01;
                     // TTL: 60s
                     rx_buffer[ptr++] = 0x00; rx_buffer[ptr++] = 0x00; rx_buffer[ptr++] = 0x00; rx_buffer[ptr++] = 0x3C;
                     // RDLENGTH: 4
                     rx_buffer[ptr++] = 0x00; rx_buffer[ptr++] = 0x04;
                     // RDATA: IP Address
                     memcpy(&rx_buffer[ptr], &my_ip, 4);
                     ptr += 4;
                     
                     sendto(sock, rx_buffer, ptr, 0, (struct sockaddr *)&source_addr, socklen);
                 }
             }
         }
         close(sock);
...
void update_leds(uint8_t global_brightness) {
    ...
    for(int i=0; i<LED_STRIP_NUM_LEDS; i++) {
        led_strip_set_pixel(led_strip, i, led_colors[i].r*scale, led_colors[i].g*scale, led_colors[i].b*scale);
    }
    led_strip_refresh(led_strip);
}
...
void app_main(void)
{
    ...
    // Start Tasks
    // 1. Serial (Highest Prio was 10, now Monitor is higher)
    xTaskCreate(task_serial, "serial", 4096, NULL, 10, NULL);
    // 2. Monitor (Controls Wi-Fi State) - Increased priority to 12 prevents starvation by Serial
    xTaskCreate(task_monitor, "monitor", 5120, NULL, 12, NULL);
    // 3. UDP (Only runs when Wi-Fi enabled)
    ...
    // Boot Anim
    for(int i=0; i<30; i++) {
         led_strip_set_pixel(led_strip, i, 50, 0, 0); led_strip_refresh(led_strip); vTaskDelay(pdMS_TO_TICKS(10));
    }
    led_strip_clear(led_strip); led_strip_refresh(led_strip);
    
    ESP_LOGI(TAG, "System Ready. Waiting for Serial or Timeout...");
}
...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data) {
    ...
    if (event->event_id == MQTT_EVENT_CONNECTED) {
        ...
        snprintf(g_device_id, sizeof(g_device_id), "AF%02X%02X%02X", mac[3], mac[4], mac[5]);
        ...
        snprintf(status_topic, sizeof(status_topic), "alfred/devices/%s/status", g_device_id);
        esp_mqtt_client_publish(mqtt_client, status_topic, "{\"status\":\"online\"}", 0, 1, 0);
        ...
        if (strstr(topic, "/power") && !strstr(topic, "/state")) {
             if (payload[0] == '1' || strstr(payload, "true")) g_home_power = true;
             else g_home_power = false;
        ...
        // 2. BRIGHTNESS COMMAND
             ...
             int b = atoi(payload);
             if(b >= 0 && b <= 100) g_home_bri = b;
             ESP_LOGI(TAG, "MQTT Brightness CMD: %d", g_home_bri);

             // FEEDBACK: Publish State
             char state_topic[64];
             snprintf(state_topic, sizeof(state_topic), "alfred/devices/%s/brightness/state", g_device_id);
             char b_str[8]; sprintf(b_str, "%d", g_home_bri);
             esp_mqtt_client_publish(mqtt_client, state_topic, b_str, 0, 1, 0);
        }
        // 3. COLOR COMMAND
        else if (strstr(topic, "/color") && !strstr(topic, "/state")) {
             sscanf(payload, "%hhu,%hhu,%hhu", &g_home_color.r, &g_home_color.g, &g_home_color.b);
             ESP_LOGI(TAG, "MQTT Color CMD: %d,%d,%d", g_home_color.r, g_home_color.g, g_home_color.b);
...
void start_mqtt() {
    ...
    snprintf(lwt_topic, sizeof(lwt_topic), "alfred/devices/%s/status", g_device_id);

    esp_mqtt_client_config_t mqtt_cfg = {
        .broker.address.uri = g_mqtt_uri,
        .credentials.username = (strlen(g_mqtt_user) > 0) ? g_mqtt_user : NULL,
        .credentials.authentication.password = (strlen(g_mqtt_pass) > 0) ? g_mqtt_pass : NULL,
        // LWT Configuration
